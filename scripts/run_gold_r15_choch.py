"""R15 — CHoCH (Change of Character) — DEV grid + sacred DEV/HOLDOUT + cost stress.

docs/research/R15_R17_SMC_PLAN.md, R15.

Mandatory baseline per plan §7: R12 (ATR-range breakout, already falsified) —
both are "stop-run past a level" mechanisms, differing only in how the
trigger level is defined (swing structure vs raw ATR multiple). If R15 does
not clear R12 by a meaningful margin, the swing-based level adds nothing.

  PYTHONPATH=. .venv/bin/python -u scripts/run_gold_r15_choch.py           # 3y smoke
  PYTHONPATH=. .venv/bin/python -u scripts/run_gold_r15_choch.py --full    # 2006-2018 DEV grid
  PYTHONPATH=. .venv/bin/python -u scripts/run_gold_r15_choch.py --holdout # sacred DEV/HOLDOUT (single run)
"""
...
# check) -- see docs/research/artifacts/xau_r15_choch.txt once it exists.
FROZEN_CFG: dict = dict(w=5, k_range=2.0, tp_r_mult=1.5, direction="both")
...
def run_signals(m15, m1, spec, **cfg):
    sig = generate_r15_signals(m15, **cfg)
...
def sacred_holdout(spec):
    print(f"[R15 HOLDOUT] frozen config: {FROZEN_CFG}")
    ...
    print("\n########## GATE CHECK (pre-committed, same discipline as R2/R8/R11/R14) ##########")
