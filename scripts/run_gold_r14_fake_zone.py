"""R14 — Level-Anchored Liquidity Sweep (fake breakout reversal) — DEV grid +
sacred DEV/HOLDOUT + cost stress.

docs/research/XAU_REDDIT_SCOUT.md ("รอบ Facebook" section, R14).

NOTE on the R8/R11-beating gate: R14's original plan required beating R11/R8
baselines significantly before being worth coder time. Both R11 and R8 are
now themselves FALSIFIED (net-losing) strategies, so "beat a losing strategy"
is not a meaningful bar. Per explicit instruction, this run drops that
requirement and instead runs R14 through the full standard pipeline (DEV grid
-> sacred holdout -> cost stress) using the track's normal gate.

  PYTHONPATH=. .venv/bin/python -u scripts/run_gold_r14_fake_zone.py           # 3y smoke
  PYTHONPATH=. .venv/bin/python -u scripts/run_gold_r14_fake_zone.py --full    # 20y-DEV-window grid
  PYTHONPATH=. .venv/bin/python -u scripts/run_gold_r14_fake_zone.py --holdout # sacred DEV/HOLDOUT (single run)
"""
...
# Frozen config — filled in by hand after inspecting the DEV grid gradient
# (see docs/research/artifacts/xau_r14_fake_zone.txt). Placeholder until then.
FROZEN_CFG: dict = dict(w=3, b_break=0.5, N=3, tp_r_mult=1.5, direction="both")
...
def run_signals(m15, m1, spec, **cfg):
    sig = generate_r14_signals(m15, **cfg)
...
def sacred_holdout(spec):
    print(f"[R14 HOLDOUT] frozen config: {FROZEN_CFG}")
    ...
    print("\n########## GATE CHECK (pre-committed, same discipline as R2/R8/R11) ##########")
