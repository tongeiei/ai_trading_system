...
- **`.env` required keys**: `BINANCE_TESTNET_API_KEY`, `BINANCE_TESTNET_API_SECRET`,
  optionally `DISCORD_WEBHOOK_URL` (alerting — not yet configured, see
...
src/live/
  ...
  alerting.py          Discord webhook alerts (needs DISCORD_WEBHOOK_URL, not yet set)
...
User chose **Discord** for alerting. Code is written and deployed
...
1. User creates a Discord webhook (Server → Channel → Edit → Integrations →
   Webhooks → New Webhook → copy URL)
2. User adds `DISCORD_WEBHOOK_URL=...` to `.env` on the dev Mac themselves
   (assistant should not see/type the URL — same handling as the Binance
   API keys)
3. Assistant scp's the updated `.env` to the VPS (same pattern used for the
   Binance keys — copy the file, never read its contents)
4. Restart `signal-cycle.service` on the VPS, verify a test alert fires
